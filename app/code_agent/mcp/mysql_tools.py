...
READ_ONLY_MODE = os.getenv("MYSQL_READ_ONLY", "true").lower() == "true"

def validate_write_permission(command: str):
    if READ_ONLY_MODE:
        lower_cmd = command.lower()
        write_keywords = ["insert", "update", "delete", "drop", "truncate", "create", "alter"]
        if any(w in lower_cmd for w in write_keywords):
            raise PermissionError(f"Security Alert: System is in READ_ONLY_MODE. Data mutation commands are blocked: {command}")
...
def execute_query(command, database = None, params = None, commit = False):
    validate_write_permission(command)
